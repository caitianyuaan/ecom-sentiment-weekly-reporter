...
首次运行不会直接报“找不到配置文件”，而是提供两个入口：

- 使用推荐配置立即生成：美国 + 英国、主流平台、全部观测指标、均衡来源、中文紧凑 HTML、仅保存本地。
- 自定义配置：依次选择市场与平台、新闻主题与观测指标、来源策略、文件格式与发送渠道。

文件格式和发送渠道分别配置：文件可选择 HTML、Markdown 或两者同时生成；渠道可选择仅本地、飞书群、飞书文档或群与文档同时使用。只有选择飞书渠道后才需要提供对应地址和署名。
...
| `platforms` | 重点平台列表 |
| `topics` | 重点议题列表 |
| `observation_metrics` | 观测指标分组，如商品供给、内容发现、物流履约、售后信任和经营增长 |
